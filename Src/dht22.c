...
//			cun[5] = 3;
		...
		if (time >= 70.0 && time < 90.0) {
			handle->bitsRX[handle->bitPos / 8] &= ~(1
					<< (7 - handle->bitPos % 8));
			cun[handle->bitPos] = 0;
			//cun[(handle->bitPos)+100]=handle->bitPos;
			handle->bitPos++;
		} else if (time > 110.0 && time < 135.0) {
	...
	if(handle->bitPos==40){
		...
		if (sum == handle->bitsRX[4]) {
			...
			if ((handle->bitsRX[2] & 0x80) == 0x80) {
				...
				temp10 |= handle->bitsRX[2];
				//temp10 *= -1;
			} else {
				//temp10 |= handle->bitsRX[] << 8;
				temp10 |= handle->bitsRX[2];
			}
			handle->temp = temp10;
